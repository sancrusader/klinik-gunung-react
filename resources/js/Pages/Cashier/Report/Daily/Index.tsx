...
export default function Index({ payments, paymentsCount }: Props) {
    const totalAmountPaid = Array.isArray(payments)
    ? payments.reduce((sum, payment) => sum + Number(payment.amount_paid || 0), 0)
    : 0;

    payments.forEach(payment => console.log("Amount Paid:", payment.amount_paid));
  ...
  return (
    <CashierSidebar header={'Daily Report'}>
      <PageContainer>
        ...
        <Card className="w-full">
          <CardHeader>
            <CardTitle>Daily Report Payments</CardTitle>
          </CardHeader>
          <CardContent>
            {payments.length > 0 ? (
              <Table>
